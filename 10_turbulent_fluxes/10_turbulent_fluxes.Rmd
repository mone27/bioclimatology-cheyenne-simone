...
CO2 flux. In this chapter, we learned some ways to trace the flux of
...
time. The flux of CO2 and H2O is measured with $\mu mol\ m^{-2} s{-1}$ and $mmol\ m^{-2} s{-1}$ respectively. Leaf evapotranspiration and Heat is measured by $W m^{-2}$.
...
$$F_c = \frac{dc}{dt} \frac{M*V}{A}$$\$

where:
 - $dc$: change of CO2 concentration ($\mu mol\ mol^{-1}$)
