...
set.seed(100)
# taking 4 sample data for plots
ec_samples_paths <- sample(dir_ls("../Data_lectures/10_Turbulent_fluxes_II/10_Turb_fluxes_CO2/"), 4)
...
```{r}
plots <- map(ec_samples, function(ec){
  co2_mean <- mean(ec$CO2, na.rm=T)
...
max_n_cov <- function(w, gas){
  opt <- optimize(function(n){
    ...
    lagged_cov(w, gas, as.integer(n)) 
  }, interval = c(-2000, 2000), maximum = T, tol=1)
  return(opt$maximum)
}
...
```{r}
plots <- map(ec_samples, function(ec){
  co2_lagged_cov <- tibble( n = seq(-2000, 2000), 
